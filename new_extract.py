    ...
    with open(main_dir + bank, encoding="utf-8") as f:
        ...
        while True:
            ...
            try:
                ...
                result.append((sent_id, derivations, header, header + original_cfg,
...
def extract_sync_grammer(java_out_dir, output_file, output_fulllabel_file, output_source,
    ...
    def convert_derivation(derivation):
        ...
        for rule in derivation:
            ...
            if rule is None:
                ...
                standard_rule = all_rules_temp.get(rule)
                if standard_rule is None:
                    rule_to_ix[rule] = len(all_rules)
                    all_rules.append(rule)
                    standard_rule = all_rules_temp[rule] = rule
            yield (rule_to_ix[standard_rule], full_rule)
    ...
    with open("deepbank-preprocessed/results_list-{}.pickle".format(name), "wb") as f:
        pickle.dump(results_list, f)
    
    f=open(output_source, "w")
    for bank, results in results_list:
        for result in results:
            sent_id, derivation, header, original_cfg, additional_cfg = result
            sentence=header.split("  ")
            ...
            counter=0
            for ix, rule in convert_derivation(derivation):
                if (isinstance(rule.rhs[0][0],Lexicon)):
                    f.write("SHIFT-")
                    assert(sentence[counter]==rule.rhs[0][0].string)
                    counter+=1
                elif(len(rule.rhs)==1):
                    f.write("UNARY-")
                elif(len(rule.rhs)==2):
    ...
    with open("deepbank-preprocessed/word_list.pickle","wb") as f:
        pickle.dump(all_words,f)
    
    with open("deepbank-preprocessed/word_to_ix.pickle","wb") as f:
        pickle.dump(word_to_ix,f)
    
    with open("deepbank-preprocessed/rule_to_ix.pickle","wb") as f:
        pickle.dump(rule_to_ix,f)

    with open("deepbank-preprocessed/rule_list.pickle","wb") as f:
...
def extract_banks(prefix, strip_tree=DONT_STRIP, graph_type="eds",
    ...
    for mode in ("train", "dev", "test"):
        ...
        output_source = "./deepbank-preprocessed/" + "src." + mode
        output_fulllabel_file = "./deepbank-preprocessed/" + prefix + ".fulllabel." + mode
        extract_sync_grammer(main_dir, output_file, output_fulllabel_file, output_source,
